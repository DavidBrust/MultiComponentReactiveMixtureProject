...
function prism_sq(data; nref=0, w=data.wi, h=data.h, cath=data.cath, catwi=data.catwi)
	...
	hmin=h_/50.0
    hmax=h_/10.0
 	H=geomspace(0.0,h_,hmax,hmin)
    # add thin, unreactive layer on top, attempt to fix partial pressure at the inlet
	ε_fix = 10.0e-3*ufac"mm"
	Hfix = glue( H, [h_, h_ + ε_fix])
	#ytop = glue(yfix, linspace(yfix[end], Wlow, nylow + 1))

	grid=simplexgrid(W,W,Hfix)
	#grid=simplexgrid(W,W,H)
	
	# catalyst layer region
	cellmask!(grid,[0.0,0.0,h],[catwi/2,catwi/2,h_],2)
	# catalyst layer boundary
	#bfacemask!(grid,[0.0,0.0,h+cath],[catwi/2,catwi/2,h+cath],Γ_top_cat)
	bfacemask!(grid,[0.0,0.0,Hfix[end]],[catwi/2,catwi/2,Hfix[end]],Γ_top_cat)	
end
...
Base.@kwdef mutable struct ModelData <:AbstractModelData
	...
	#Qflow::Float64=34.0*ufac"ml/minute" # volumetric feed flow rate (sccm)
...
function main(;data=ModelData())
	...
	sol=solve(sys;inival,)
	 ...
	 function pre(sol,par)
	 	...
	 	mcats=[10.0, 130.0]*ufac"kg/m^3"
		 #data.mcats= minimum(mcats) + par*(maximum(mcats)-minimum(mcats))
		#mcats=[10.0, 0.0013]*ufac"kg/m^3"
	 	data.mcats= minimum(mcats) + par*(maximum(mcats)-minimum(mcats))
		...
		# pore size
	 	#dps=[200.0, 20.0]*ufac"μm"
	 	#data.dp= maximum(dps) + par*(minimum(dps)-maximum(dps))

		 # porosity
	 	#ϕs=[0.33, 0.2]*ufac"μm"
	 	#data.ϕ= maximum(ϕs) + par*(minimum(ϕs)-maximum(ϕs))
...
#=╠═╡
md"""
Cutplane at ``z=`` $(@bind zcut PlutoUI.Slider(range(0.0,data_embed.h+2*data_embed.cath,length=1001),default=data_embed.h,show_value=true)) m
"""
  ╠═╡ =#
...
let
	...
	gridplot!(vis, prism_sq_(ModelData()), zplane=zcut)
	reveal(vis)
end
  ╠═╡ =#
...
let
	...
	#lens!([0.5, .55001], [0.45, 0.55], inset = (1, bbox(0.15, 0.1, 0.5, 0.5)))
	p
...
# ╔═╡ 0db38828-75a0-4d5b-9373-1d2f0975bc8b
#=╠═╡
let
 	(;Tn,pn,Qflow,Fluids,X0,u0,Ac,ubot) = data_embed
	ndot=pn*Qflow/(ph"R"*Tn)
	Mmix=molarweight_mix(Fluids, X0)
	ndot*Mmix
	ndot_top = u0*pn/(ph"R"*Tn)*Ac/ufac"mol/hr"/4
end
  ╠═╡ =#
...
|    | Top   | Bottom    |    |
|----|-------|-------|-------|
| $(data_embed.gn[1]) | $(abs(round(ndot_top[1]/ufac"mol/hr",sigdigits=3)))  |   $(round(ndot_bot[1]/ufac"mol/hr",sigdigits=3))   |  mol/hr  |
| $(data_embed.gn[2]) | $(abs(round(ndot_top[2]/ufac"mol/hr",sigdigits=3)))  |   $(round(ndot_bot[2]/ufac"mol/hr",sigdigits=3))   |  mol/hr  |
| $(data_embed.gn[3]) | $(abs(round(ndot_top[3]/ufac"mol/hr",sigdigits=3)))  |   $(round(ndot_bot[3]/ufac"mol/hr",sigdigits=3))   |  mol/hr  |
| $(data_embed.gn[4]) | $(abs(round(ndot_top[4]/ufac"mol/hr",sigdigits=3)))  |   $(round(ndot_bot[4]/ufac"mol/hr",sigdigits=3))   |  mol/hr  |
| $(data_embed.gn[5]) | $(abs(round(ndot_top[5]/ufac"mol/hr",sigdigits=3)))  |   $(round(ndot_bot[5]/ufac"mol/hr",sigdigits=3))   |  mol/hr  |
