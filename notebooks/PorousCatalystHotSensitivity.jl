...
function mole_frac!(node,data,X,u)
	...
	for i=1:n
		...
		if u isa VoronoiFVM.EdgeUnknowns
			X[i] = 0.5*(u[i,1]+u[i,2])
		#elseif node isa VoronoiFVM.BNode # boundary node
...
function main(;data=ModelData())
	...
	function pre(sol,par)
		...
		function Inttop(f,u,bnode,data)
			...
			X=zeros(eltype(u), ng)
			mole_frac!(bnode,data,X,u)
			if bnode.region==3 || bnode.region==5 # top boundary(cat/frit)
				for i=1:ng
		...
		MWavg=sum(integrate(sys,Inttop,sol; boundary=true)[1:ng,[3,5]])/data.Ac		
		ntop=data.mdotin/MWavg
		
		Tavg=sum(integrate(sys,Inttop,sol; boundary=true)[data.iT,[3,5]])/data.Ac
