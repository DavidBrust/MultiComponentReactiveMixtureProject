...
begin
	...
	using VoronoiFVM, LinearSolve,ExtendableGrids
	using StaticArrays
	using LinearAlgebra
	using GridVisualize
	using PlutoVista
...
# ╔═╡ 2f2b18b0-bd72-4720-90f1-2cc9b387e347
md"""
...
```math
  \begin{aligned}
\partial_t u_i +  \nabla \cdot \vec N_i &=R_i \quad (i=1\dots n)\\
  \frac{\vec N_i}{D^K_i} + \sum_{j\neq i}\frac{x_j \vec N_i - x_i \vec N_j}{D^B_{ij}} &= p \vec \nabla x_i + \frac{D}{D_i^K}x_i\vec\nabla p \quad (i=1\dots n)\\
  x_i&=\frac{u_i}{p}\\
    \sum_{i=1}^n u_i=p
  \end{aligned}
```
...
# ╔═╡ 3916020a-c70b-4637-b86b-5f58806b50b0
md"""
```math
\begin{aligned}
 \frac{\vec N_i}{D^K_i} + \sum_{j\neq i}\frac{x_j \vec N_i - x_i \vec N_j}{D^B_{ij}} &= p \vec \nabla x_i + \frac{D}{D_i^K}x_i\vec\nabla p \quad (i=1\dots n)\\
 \vec N_i+ D^K_i\sum_{j\neq i}\frac{x_j \vec N_i - x_i \vec N_j}{D^B_{ij}} &= D^K_ip \vec \nabla x_i + Dx_i\vec\nabla p \quad (i=1\dots n)\\
 \vec v+ \sum_i D^K_i\sum_{j\neq i}\frac{x_j \vec N_i - x_i \vec N_j}{D^B_{ij}}& = \sum_i  D^K_ip \vec \nabla x_i + D\vec\nabla p \\
\end{aligned}
```

```math
\begin{aligned}
 \vec v+ \sum_i D^K\sum_{j\neq i}\frac{x_j \vec N_i - x_i \vec N_j}{D^B_{ij}} &=  D\vec\nabla p \\
\end{aligned}
```
"""

# ╔═╡ 9a477f42-7970-4b65-9a84-3a29fe39f246
md"""
Sum up equations:
Let ``\vec v = \sum N_i``
```math
\begin{aligned}
\partial_t p + \nabla \cdot \vec v &= \sum R\\
                      \vec v + D\nabla p &=0
\end{aligned}
```
"""
...
``x_i`` are the species partial molar fraction, ``\vec N_i`` are the species fluxes.
``D_i^K`` are the Knudsen diffusion coefficients, and ``D^B_{ij}`` are the binary diffusion coefficients.
```math
  \begin{aligned}
 \partial_t (px_i) + \nabla \cdot (\vec N_i +  px_i \vec v) &=R_i \quad (i=1\dots n)\\
  \frac{\vec N_i}{D^K_i} + \sum_{j\neq i}\frac{x_j \vec N_i - x_i \vec N_j}{D^B_{ij}} &= p \vec \nabla x_i \\
    \sum_{i=1}^n x_i=1\\
    \sum_{i=1}^n N_i=0\\
...
# ╔═╡ 67f1767a-3db3-4b4e-9804-18b42bf5f730
md"""
```math
N_n= - \sum_{i<N} N_i, \quad x_n=1-\sum_{i<N} x_i
```
"""

# ╔═╡ 4af65585-a5e2-40fc-9345-0edffa5c4d73
md"""
`` i<N``
```math
\begin{aligned}
\sum_{j\neq i}\frac{x_j \vec N_i - x_i \vec N_j}{D^B_{ij}}&= 
\sum_{j\neq i,j<n}\frac{x_j \vec N_i - x_i \vec N_j}{D^B_{ij}}
+ \frac{x_n\vec{N_i} -x_i \vec{N_n}}{D_{in}^B}\\
&= 
\sum_{j\neq i,j<n}\frac{x_j \vec N_i - x_i \vec N_j}{D^B_{ij}}
+  \frac{(1-\sum_{j<n} x_j)\vec N_i + x_i\sum_{j<n} \vec{N_j}}{D_{in}^B}
\end{aligned}
```
"""

# ╔═╡ 7053489e-255c-4386-884f-0aafadc6d2a2
md"""
```math
\begin{aligned}
m_{ii}&= \sum_{j\neq i} \frac{x_j}{D_{ij}} + \frac{1-\sum_{j<n}x_j}{D^B_{in}} + \frac{x_i}{D^B_{in}}\\
&=\sum_{j\neq i} \frac{x_j}{D_{ij}} + \frac{1-\sum_{j\neq i}x_j}{D^B_{in}}\\
&=\frac{1}{D^B_{in}}+\sum_{j\neq i} x_j \left(\frac1{D_{ij}} -\frac1{D^B_{in}}\right)
\end{aligned}
```

```math
m_{ij}= \frac{x_i}{D^B_{in}}-\frac{x_i}{D^B_{ij}}
```
"""

# ╔═╡ 2922c084-df64-4759-b6a9-3a17cbe7ed0a
md"""
From this representation, we can derive the matrix ``M=(m_{ij})`` with
```math
 \begin{aligned}
 m_{ii}= \frac{1}{D^K_i} + \sum_{j\neq i} \frac{x_j}{D_{ij}}\\
m_{ij}= -\frac{x_i}{D_{ij}}
  \end{aligned}
```
such that 
```math
...
# ╔═╡ 5c978104-3bf9-4e6e-81a5-0a8998b4e539
struct MyData{NSPec}
    DBinary::Matrix{Float64}
...
# ╔═╡ bc2df25d-79c3-4721-8c70-baa5774e2f9f
getnspec(::MyData{NSpec}) where {NSpec} = NSpec

# ╔═╡ 16f51591-bd3a-49e6-9d30-cfc8ac1c8a5c
begin
	n=21
	dim=1
	nspec=3
...
# ╔═╡ 1cefdc21-3086-4fcd-8e51-f1802a10ac12
M=2

# ╔═╡ baaa93c2-8bc8-4b11-b5d1-d954b3a34a59
function reaction(f,u::AbstractArray{T}, node, data::MyData{NSpec}) where {T,NSpec}
  ip=NSpec
if false
	p=u[ip]
	# u1 + Mu2 <-> 2u3
	...
	c2=u[2]*p
	c3=(1-u[1]-u[2])*p
	r= 0.2*(data.kp*c1*c2^M-data.km*c3^2)
...
# ╔═╡ 6b3f3368-eb80-46cd-9bed-96e7faf116cd
function storage(f, u, node, data::MyData{NSpec}) where NSpec
	ip=NSpec+1
	 for i=1:(NSpec)
		f[i]=u[i]*u[ip]
	 end
...
# ╔═╡ 18c783ab-00cc-48da-9a34-69ef06b1f726
kp=10;km=0.1;

# ╔═╡ c5a57b69-b825-4617-a132-0a9225e50607
function bcondition(f, u, node, data::MyData{NSpec}) where NSpec
	ip=nspec
	
	boundary_dirichlet!(f, u, node; species=1, region = data.diribc[1], value=0.5)
    boundary_dirichlet!(f, u, node; species=2, region = data.diribc[1], value=0.5)
 
		boundary_dirichlet!(f, u, node; species=ip, region = data.diribc[1], 
			value=1.1)
		boundary_dirichlet!(f, u, node; species=ip, region = data.diribc[2], 
			value=0.1)
end


# ╔═╡ c4ca2b36-e4af-4475-a581-2206ecda53dd
begin
h = 1.0 / convert(Float64, n-1)
X = collect(0.0:h:1.0)
end

# ╔═╡ 01cdec46-299c-48e1-9901-a46920dd620a
strategy=nothing #DirectSolver(UMFPACKFactorization()) # GMRESIteration(UMFPACKFactorization(), EquationBlock())

# ╔═╡ 26088ee3-f4e4-4b16-ba1a-1cdf68704f9b
begin
	db=0.1
    DBinary = fill(db, nspec, nspec)
    for ispec = 1:nspec
...
# ╔═╡ 2a028954-4ae6-45a3-b135-8acc6b5b336b
 DKnudsen = fill(1,nspec)

# ╔═╡ b9bdd941-2c86-4193-91a0-e5a966d84ed4
D=0.1

# ╔═╡ b6bceda0-1c91-4001-993c-6fb9913e8258
function flux(f, u::AbstractArray{T}, edge, data::MyData{NSpec}) where {T,NSpec}
    ip=NSpec

    N=MVector{NSpec-1,T}(undef)
    xkl=MVector{NSpec-1,T}(undef)
	...
	M=MMatrix{NSpec-1,NSpec-1,T}(undef)
	for ispec = 1:(NSpec-1)
        M[ispec, ispec] = 1.0 / data.DBinary[ispec,NSpec]
        for jspec = 1:(NSpec-1)
            if jspec != ispec
                M[ispec, ispec] += xkl[jspec]*(1/data.DBinary[ispec, jspec] -1/data.DBinary[ispec, NSpec])
				M[ispec, jspec] = xkl[ispec]*( 1/data.DBinary[ispec, NSpec]- 1/data.DBinary[ispec, jspec])
            end
        end
    end
	#@inline inplace_linsolve!(M,N)
    for ispec = 1:(NSpec-1)
        f[ispec] = N[ispec] + pkl*xkl[ispec]*dpkl
	end
	f[ip]=dpkl
end
...
# ╔═╡ 478a1ac0-bbb3-47e6-938b-729991d2f211
function boutflow(y,u,edge,data::MyData{NSpec}) where NSpec
	ip=NSpec
	pkl=0.5*(u[ip,1]+u[ip,2])	

	velo=D*(u[ip,1]-u[ip,2])
	for ispec=1:(NSpec-1)
		y[ispec]= -velo*u[ispec,outflownode(edge)]*u[end,outflownode(edge)]
	end
end


# ╔═╡ b789a556-353e-40f2-a67e-e9524468609c
if dim == 1
...
# ╔═╡ 7bf2e0ee-91ff-4d79-bbcf-765e8c04d6c7
   data = MyData{nspec}(DBinary, DKnudsen, D, diribc,kp,km)
 

# ╔═╡ 4b6b1cad-23a0-4122-a1bd-46cc0fc9cd5f
function getconc(sys,sol,iconc)
	if iconc<size(sol,1)
		return sol[iconc,:].*sol[end,:]
	else
		[(1-sum(sol[1:(end-1),i]))*sol[end,i] for i=1:size(sol,2)]
	end
end

# ╔═╡ 05358bf6-50e5-4473-85e2-e9e0c5b2635c
function getfrac(sys,sol,iconc)
	if iconc<size(sol,1)
		return sol[iconc,:]
	else
		[1-sum(sol[1:(end-1),i]) for i=1:size(sol,2)]
	end
end

# ╔═╡ 16af6468-5636-4cc8-9323-56d955b1c84d
function getfracs(sys,sol)
	fracs=copy(sol)
	for i=1:size(sol,2)
		fracs[end,i]=1-sum(sol[1:(end-1),i])
	end
	fracs
end

# ╔═╡ e09c2384-43a7-4fe8-bd63-e1c39a64fa5d
getpress(sys,sol)=sol[end,:]

# ╔═╡ 85d1abe1-7e62-40f1-a6c9-5256f453611e
begin   
    sys = VoronoiFVM.System(grid; data, flux, storage, 
	boutflow,outflowboundaries=[2],
    reaction, 
	bcondition, species = 1:nspec)
	...
	u0=unknowns(sys,inival=0)
	for i=1:num_nodes(grid)
		@views u0[1:nspec-1,i].=0.5
		@views u0[nspec,i]=1
	end
	times=[0,10]
    @show "hi"
    u=solve(sys;inival=u0,control,verbose="en")
	if false
    @show "hi"
	tu[end].=solve(sys;inival=tu[end],control,verbose="n")
	end
end


# ╔═╡ 5ec05e12-91ea-4ee4-9fee-ed747edce791
#u=tu(t)

# ╔═╡ 403c3296-bc8f-43fd-a771-0b7773f92aef
begin
	tfac=TestFunctionFactory(sys)
	tfin=testfunction(tfac,[2],[1])
	tfout=testfunction(tfac,[1],[2])
end

# ╔═╡ 0e4cada8-3c46-4c5c-8be3-77cc0f74ade7
vin=integrate(sys,tfin,u)[3]

# ╔═╡ ebc4f679-2038-4945-ae4b-282bf6410ec1
vout=integrate(sys,tfout,u)[3]

# ╔═╡ b9f2fae1-4fce-4b69-b607-4e51ed5be528
x=getfracs(sys,u)

# ╔═╡ 49a1186f-8d18-42af-a39f-9daf29e0c741
flin=integrate(sys,tfin,u)

# ╔═╡ 7385cfc4-a8f2-40c3-ac6e-88add59f889c
flout=integrate(sys,tfout,u)

# ╔═╡ 8ac5b882-92a2-4124-bce7-5b1b232599f9
rea=integrate(sys,reaction,u)

# ╔═╡ 71e4b1eb-0308-4998-b9d8-7e15e0928feb
flin[2]-rea[2]+flout[2]

# ╔═╡ 2c38812a-ec2e-4b0e-a489-737471d6e4f7
rea[1]+rea[2]+flout[3]
    ...
    function floataside(text::Markdown.MD; top = 1)
        ...
        @htl("""
       	...
       	transition: transform 300ms cubic-bezier(0.18, 0.89, 0.45, 1.12);
       			...
       			aside.plutoui-aside-wrapper-$(uuid) > div {
       #				width: 300px;
       			}
       		}
       		</style>
...
# ╔═╡ 6dbe7915-122c-4167-b759-ecdcab3ab76b
floataside( flin[1]-rea[1]+flout[1], top=500)

# ╔═╡ 866d4394-cbde-4b4a-bbfc-bfecc0fc3531
floataside(@bind t Slider(range(times...,length=201),show_value=true,default=times[end]) )

# ╔═╡ 3138f72d-3170-4eba-8163-64991bb00a4d
floataside(let
	vis=GridVisualizer(size=(400,200),title="t=$(t)")
	colors=[:red,:green,:blue]
	scalarplot!(vis,grid,getpress(sys,u),color=:black,clear=false)
	for ispec=1:nspec
		scalarplot!(vis,grid,getfrac(sys,u,ispec),color=colors[ispec],clear=false)
	end
	reveal(vis)
end,top=200)
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "c89a9ca5ea44581ed81d2f6a95e80c7d9288c32a"
...
git-tree-sha1 = "02b5b753f80e3498ce0e86fb762a97bdbbcd382b"
